...
class TestLayer(unittest.TestCase):
    ...
    def test_init_layer_weights_size(self):
        self.assertEqual(self.layer.weights.size,
    ...
    def test_weights_within_range(self):
        within_range = np.where(np.logical_and(
            self.layer.weights >= 0, self.layer.weights <= 1))
    ...
    def test_backward_prop_calculate_error_gradient_weights_wrong_output(self):
        ...
        error_gradient_weights = self.layer.calculate_error_gradient_weights(
            output_error)
        correct_output = - np.array([[0.5, 0.5, 0.5],
                                    [0.5, 0.5, 0.5],
                                    [0.5, 0.5, 0.5]])
        self.assertEqual(
            (error_gradient_weights == correct_output).all(), True)

    def test_backward_prop_calculate_error_gradient_input(self):
        error_gradient_output = np.array([[0.5, 0.5, 0.5]])
        error_gradient_input = self.layer.calculate_error_gradient_input(
            error_gradient_output)
    ...
    def test_backward_prop_calculate_error_gradient_input_right_output(self):
        self.layer.input = np.array([[1, 1, 1]])
        error_gradient_output = np.array([[0, 0, 0]])
        error_gradient_weights = self.layer.calculate_error_gradient_input(
            error_gradient_output)
        correct_output = np.zeros((3, 3))
        self.assertEqual(
            (error_gradient_weights == correct_output).all(), True)
    ...
    def test_backward_prop_adjust_weights(self):
        ...
        error_gradient_weights = np.array([[0.5, 0.5, 0.5],
                                           [0.5, 0.5, 0.5],
                                           [0.5, 0.5, 0.5]])
        self.layer.backward_propagation_adjust_weights(
    ...
    def test_backward_prop_adjust_biases(self):
        learning_rate = 0.1
        error_gradient_output = np.array([[0.5, 0.5, 0.5]])
        self.layer.backward_propagation_adjust_biases(
            error_gradient_output, learning_rate)

...
class TestError(unittest.TestCase):
    ...
    def test_mse_correct_output(self):
        ...
        self.assertEqual(
            loss_functions.mean_squared_error(y_true, y_pred), 0.0)

    def test_mse_wrong_output(self):
        y_true = np.array([1, 0, 0])
        y_pred = np.array([0.5, 0.5, 0.5])
        self.assertEqual(
            loss_functions.mean_squared_error(y_true, y_pred), 0.25)

    def test_mse_wrong_output_random(self):
        y_true = np.random.rand(3,)
        y_pred = np.random.rand(3,)
        self.assertNotEqual(
            loss_functions.mean_squared_error(y_true, y_pred), 0)
